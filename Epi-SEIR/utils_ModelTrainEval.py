...
def train(loader, data, model, criterion, optim, batch_size, modelName, Lambda):
    ...
    for inputs in loader.get_batches(data, batch_size, True):
        ...
        if modelName == "CNNRNN_Res_epi":
            output, EpiOutput, _, _, _ = model(X);
        elif modelName == "SEIR_PIN":
            # assume backbone returns prediction compatible with previous code
            output = model(X)
        else:
            output = model(X);
        ...
        # --------------------------------------------
        # modified loss with epidemiological / physics constraints
        if modelName == "CNNRNN_Res_epi":
            loss = criterion(output * scale, Y * scale) + Lambda*criterion(EpiOutput * scale, Y * scale);
        elif modelName == "SEIR_PIN":
            # compute physics residual (model should implement compute_physics_loss)
            phys_loss = model.compute_physics_loss(X, output, Y=Y, dt=1.0)
            loss = criterion(output * scale, Y * scale) + Lambda * phys_loss
        else:
