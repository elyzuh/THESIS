...
def PlotMatrix(Matrix, Type, SaveName, save_dir):
    """
    Plots a single matrix (NGM or Mobility) as a clean heatmap.
    Used by PlotAllMatrices() in the original Liu et al. code.
    """
    ...
    if Type in ["Next Generation Matrix", "Human Mobility Matrix"]:
        ...
        cmap = 'RdBu_r'   # fallback for beta, gamma maps, etc.
    ...
    # Put x-axis labels on top (standard in epidemiology papers)
    plt.tick_params(axis='x', top=True, bottom=False, labeltop=True, labelbottom=False)
    
    # Optional: uncomment if you want a title
    # plt.title(Type, fontsize=14, pad=20)

    # Save as high-quality PDF
    plt.savefig(f"{save_dir}{SaveName}.pdf",
                bbox_inches='tight',
                transparent=True,
                dpi=300)
    plt.close()
